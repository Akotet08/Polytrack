...
class TracknRecord():
    ...
    def __init__(self,
        ...
        if self.compressed_video:
            _, _, self.full_frame_numbers = self.TrackInsects.get_compression_details(self.video_source,
                                                                                      self.info_filename)
    ...
    def run(self) -> None:
        ...
        while True:
            ...
            if frame is not None:
                ...
                fgbg_associated_detections, dl_associated_detections, missing_insects, new_insects = self.TrackInsects.run_tracker(
                    frame, nframe, predicted_position)
                for_predictions = self.RecordTracks.record_track(frame, nframe, mapped_frame_num,
                                                                 fgbg_associated_detections, dl_associated_detections,
                                                                 missing_insects, new_insects)
                predicted_position = self.TrackInsects.predict_next(for_predictions)

                if ((self.compressed_video and (nframe in self.full_frame_numbers)) or (not self.compressed_video and (
                        nframe == 5 or nframe % self.flower_detection_interval == 0))) and self.TrackFlowers is not None:
                    associated_flower_detections, missing_flowers, new_flower_detections = self.TrackFlowers.run_flower_tracker(
                        frame, flower_predictions)
                    flower_detections_for_predictions, latest_flower_positions = self.RecordFlowers.record_flowers(
                        mapped_frame_num, associated_flower_detections, missing_flowers, new_flower_detections)
                ...
                if (len(for_predictions) > 0) and self.RecordFlowers is not None:
                    ...
                    self.RecordFlowers.record_flower_visitations(insect_flower_visits, mapped_frame_num,
                                                                 self.RecordTracks.insect_tracks)
